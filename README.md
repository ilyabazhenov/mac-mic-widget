...
`MicWidget` — лёгкое приложение для строки меню macOS, которое показывает текущий уровень входного микрофона и позволяет быстро переключать mute/unmute.
...
1. Открой пакет в Xcode:
   - `open Package.swift`
2. Выбери схему `MicWidget` и запусти.
