...
- Дефолтный глобальный хоткей для toggle: `Control + Option + M` (`⌃⌥M`).
...
Исходная картинка иконки хранится в `assets/AppIcon.source.png`. После её замены пересобери PNG iconset и `.icns`:
